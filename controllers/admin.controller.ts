...
import { NotificacionEstadoMotorizado, NotificacionEstadoPedido, NotificacionTopico, NotificacionTopicoPromo, NotificacionTopicoTodos } from '../classes/fcm';
...
export async function LoginAdmin(req: Request, res: Response): Promise<Response> {
    if (req.body.Usuario != undefined && req.body.Password != undefined) {
        ...
        try {
            ...
            const cliente = await conn.query("SELECT Seg_Usuario.*,Seg_UsuarioPerfil.UsuarioPerfil, Gen_Region.LatitudIni, Gen_Region.LongitudIni, Gen_Region.NombreRegion FROM Seg_Usuario "+
            ...
            if (resultado.length > 0) {
                ...
                if (password == passwordBody) {
                    let token = await generarToken(resultado[0].IdUsuario,resultado[0].NombreUsuario);
...
export async function EnivarMensaje(req: Request, res: Response): Promise<Response> {
    
    let respuestaEnviar = await EnviarOtp('+51958737428','Su código de verificación es: 3233','TAKXthTjhX8');
   
    return res.json(respuestaEnviar);
    
}

export async function Categorias(req: Request, res: Response): Promise<Response> {
    try {
        let conn = await database.conexionObtener();
        let resultadoEmpresa = await conn.query("SELECT IdEmpresaCategoria,EmpresaCategoria,ImagenUrl FROM Gen_EmpresaCategoria ORDER BY Orden ASC");
...
export async function SubCategorias(req: Request, res: Response): Promise<Response> {
    try {
        let conn = await database.conexionObtener();
        let resultadoEmpresa = await conn.query("SELECT IdEmpresaSubCategoria,SubCategoria,FechaReg,Anulado FROM Gen_EmpresaSubCategoria");
        let resultadoEmpresaFinal = JSON.parse(JSON.stringify(resultadoEmpresa[0]));
...
export async function Regiones(req: Request, res: Response): Promise<Response> {
    try {
        let conn = await database.conexionObtener();
        let resultadoRegiones = await conn.query("SELECT IdRegion, NombreRegion, LatitudIni, LongitudIni, FechaReg, UsuarioReg,Anulado FROM Gen_Region");
...
export async function Empresas(req: Request, res: Response): Promise<Response> {
    if(req.query.IdCategoria != undefined && req.query.DepartamentoUbicacion != undefined ){
        ...
        try {
            let conn = await database.conexionObtener();
            let resultadoEmpresa = await conn.query("SELECT Gen_Empresa.*,CONCAT('"+rutaFinal+"',Gen_Empresa.ImagenUrl) AS ImagenUrl,Gen_EmpresaSubCategoria.SubCategoria,IFNULL(Gen_Empresa.IdRegion,0) AS IdRegion FROM Gen_Empresa " +
...
export async function EmpresasIds(req: Request, res: Response): Promise<Response> {
    if(req.query.Anulado != undefined && req.query.Ubicacion != undefined  ){
        ...
        try {
            let conn = await database.conexionObtener();
            let resultadoEmpresa = await conn.query("SELECT IdEmpresa,RazonSocial,DepartamentoUbicacion,TieneSistema, CONCAT('"+rutaFinal+"',ImagenUrl) AS ImagenUrl FROM Gen_Empresa WHERE DepartamentoUbicacion='"+ubicacion+"'"+(anulado=='1'?" AND Anulado = 0":"")+" ORDER BY RazonSocial ASC");
            let resultadoEmpresaFinal = JSON.parse(JSON.stringify(resultadoEmpresa[0]));
...
export async function EmpresaAnular(req: Request, res: Response): Promise<Response> {
    if (req.body.IdEmpresa != undefined && req.body.Accion != undefined) {
        let idEmpresa = req.body.IdEmpresa;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado,RazonSocial FROM Gen_Empresa WHERE IdEmpresa = "+idEmpresa+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].Anulado;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_Empresa SET  Anulado = "+accion+" WHERE IdEmpresa ="+idEmpresa);    
...
export async function Anuncios(req: Request, res: Response): Promise<Response> {
...
export async function AnuncioEditar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdAnuncio != undefined && req.body.Data != undefined){
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoAnuncio = await conn.query("SELECT ImagenUrl FROM Gen_Anuncio WHERE IdAnuncio = "+idAnuncio);
            ...
            if(imagen == ''){
                let insertar = await conn.query("UPDATE Gen_Anuncio SET Empresa ='', Nombre = '"+nombre+"' ,Descripcion ='"+descripcion+"', Departamento = '"+departamento+"',"+
                ...
                fs.writeFile(join(__dirname, '/../public/img/anuncios/') + archivoNombre, base64Image, { encoding: 'base64' }, function (err) {
                });
                let insertar = await conn.query("UPDATE Gen_Anuncio SET Empresa ='', Nombre = '"+nombre+"' ,Descripcion ='"+descripcion+"', Departamento = '"+departamento+"',"+
                " FechaInicio = '"+fechaInicio+"' ,FechaFin = '"+fechaFin+"' ,ImagenUrl='"+archivoNombre+"' WHERE IdAnuncio = "+idAnuncio);
                if(archivoAnterior!= ''){
                    fs.unlink(join(__dirname, '/../public/img/anuncios/') + archivoAnterior, (err) => {
                    });
                }
                return res.json({ success: true });
            }
...
export async function AnuncioAnular(req: Request, res: Response): Promise<Response> {
    if (req.body.IdAnuncio != undefined && req.body.Accion != undefined) {
        let idAnuncio = req.body.IdAnuncio;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado,Nombre FROM Gen_Anuncio WHERE IdAnuncio = "+idAnuncio+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].Anulado;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_Anuncio SET  Anulado = "+accion+" WHERE IdAnuncio ="+idAnuncio);    
...
export async function AnuncioEliminar(req: Request, res: Response): Promise<Response> {
    if (req.body.IdAnuncio != undefined) {
        let idAnuncio = req.body.IdAnuncio;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado, Nombre, ImagenUrl FROM Gen_Anuncio WHERE IdAnuncio = "+idAnuncio+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let resultado1 = await conn.query("DELETE FROM Gen_Anuncio WHERE IdAnuncio = "+idAnuncio);
                if(resultado[0].ImagenUrl != '' && resultado[0].ImagenUrl != null){
                    fs.unlink(join(__dirname, '/../public/img/anuncios/') + resultado[0].ImagenUrl, (err) => {
                    });
                } 
...
export async function AnuncioNuevo(req: Request, res: Response): Promise<Response> {
    if(req.body.Data != undefined){
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Orden FROM  Gen_Anuncio WHERE Departamento = '"+departamento+"' ORDER BY Orden DESC LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
            ...
            let insertar = await conn.query("INSERT INTO Gen_Anuncio (Empresa, Nombre, Descripcion, Departamento, FechaInicio, FechaFin, ImagenUrl, Orden, Anulado) VALUES ('', '"+nombre+"','"+descripcion+"', '"+departamento+"',"+
...
export async function AnuncioOrden(req: Request, res: Response): Promise<Response> {
    if (req.body.IdAnuncio != undefined && req.body.Orden != undefined) {
        let idAnuncio = req.body.IdAnuncio;
        let orden = req.body.Orden;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT IdAnuncio FROM Gen_Anuncio WHERE IdAnuncio = "+idAnuncio+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                    let resultado1 = await conn.query("UPDATE Gen_Anuncio SET  Orden = "+orden+" WHERE IdAnuncio ="+idAnuncio);    
...
export async function Departamentos(req: Request, res: Response): Promise<Response> {
    try {
        let conn = await database.conexionObtener();
        let resultadoDepa = await conn.query("SELECT DISTINCT DepartamentoUbicacion FROM Gen_Empresa");
...
export async function Motorizados(req: Request, res: Response): Promise<Response> {
    if(req.query.Busqueda != undefined && req.query.IdEmpresa != undefined && req.query.Ubicacion != undefined && req.query.IdGrupoMotorizado !=undefined){
        ...
        try {
            let conn = await database.conexionObtener();
            let resultadoMotorizado = await conn.query("SELECT COUNT(Empresa_Has_Motorizado.IdMotorizado) AS Empresas,Gen_Motorizado.*, IFNULL(Gen_GrupoMotorizado.IdGrupoMotorizado,0) AS IdGrupoMotorizado ,Gen_GrupoMotorizado.NombreGrupo FROM Gen_Motorizado "+
...
export async function MotorizadoEditar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdMotorizado != undefined && req.body.Data != undefined){
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoMotorizado = await conn.query("SELECT * FROM Gen_Motorizado WHERE Dni = '"+dni+"' AND IdMotorizado <> "+idMotorizado);
            let resultadoMotorizadoFinal = JSON.parse(JSON.stringify(resultadoMotorizado[0]));

            if (resultadoMotorizadoFinal.length > 0) {
                return res.json({ success: false, msg:'El dni ya ingresado ya existe en otro usuario, deben ser unicos', data:{} });
            } else {
                let insertar = await conn.query("UPDATE Gen_Motorizado SET Nombres = '"+nombres+"', Apellidos = '"+apellidos+"', Dni = '"+dni+"', Telefono = '"+telefono+"', Password = '"+password+"',"+
...
export async function MotorizadoAnular(req: Request, res: Response): Promise<Response> {
    if (req.body.IdMotorizado != undefined && req.body.Accion != undefined) {
        let idMotorizado = req.body.IdMotorizado;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado,Nombres FROM Gen_Motorizado WHERE IdMotorizado = "+idMotorizado+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].Anulado;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_Motorizado SET  Anulado = "+accion+" WHERE IdMotorizado ="+idMotorizado);    
...
export async function MotorizadoNuevo(req: Request, res: Response): Promise<Response> {
    if(req.body.Data != undefined){
        let nombres = req.body.Data.Nombres
        let apellidos = req.body.Data.Apellidos
        let dni = req.body.Data.Dni
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoMotorizado = await conn.query("SELECT * FROM Gen_Motorizado WHERE Dni = '"+dni+"'");
            let resultadoMotorizadoFinal = JSON.parse(JSON.stringify(resultadoMotorizado[0]));

            if (resultadoMotorizadoFinal.length > 0) {
                return res.json({ success: false, msg:'El dni ya ingresado ya existe en otro usuario, deben ser unicos', data:{} });
            } else {
                let insertar = await conn.query("INSERT INTO Gen_Motorizado(Nombres,Apellidos,Dni,Telefono,Password,Email,Direccion,"+
...
export async function MotorizadoEmpresas(req: Request, res: Response): Promise<Response> {
    if(req.query.IdMotorizado != undefined){
        let idMotorizado = req.query.IdMotorizado
        try {
            let conn = await database.conexionObtener();
            let queryEmpresas = await conn.query("SELECT Empresa_Has_Motorizado.IdEmpresaHasMotorizado, DATE_FORMAT(Empresa_Has_Motorizado.FechaReg, '%Y-%m-%d %H:%i:%s') AS FechaReg, Gen_Empresa.DepartamentoUbicacion, Gen_Empresa.IdEmpresa,Gen_Empresa.RazonSocial,Gen_Empresa.Anulado, Gen_EmpresaCategoria.EmpresaCategoria,Gen_Motorizado.Anulado AS MotorizadoAnulado,IFNULL(Gen_Empresa.IdRegion,0) AS IdRegion FROM Empresa_Has_Motorizado "+ 
...
export async function MotorizadoGrupos(req: Request, res: Response): Promise<Response> {
        try {
            let busqueda = req.query.Busqueda || ''
            let conn = await database.conexionObtener();
            let queryGrupos = await conn.query("SELECT Gen_GrupoMotorizado.IdGrupoMotorizado, Gen_GrupoMotorizado.NombreGrupo, Gen_GrupoMotorizado.Servicio, "+
...
export async function MotorizadoGrupoEditar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdGrupoMotorizado != undefined && req.body.Data != undefined){
        ...
        try {
            let conn = await database.conexionObtener();
            let insertar = await conn.query("UPDATE Gen_GrupoMotorizado SET NombreGrupo = '"+nombreGrupo+"', Servicio = "+servicio+", Vigente = "+vigente+", IdRegion = "+idRegion+
...
export async function MotorizadoGrupoNuevo(req: Request, res: Response): Promise<Response> {
    if(req.body.Data != undefined){
        let nombreGrupo = req.body.Data.NombreGrupo
        let servicio  = req.body.Data.Servicio
        let vigente  = req.body.Data.Vigente
        ...
        try {
            ...
            const hoyformateado = hoy.format('YYYY-MM-DD HH:mm:ss');
            let conn = await database.conexionObtener();
            let insertar = await conn.query("INSERT INTO Gen_GrupoMotorizado(NombreGrupo,Servicio,FechaReg,Vigente,IdRegion) "+
...
export async function MotorizadoRemoverEmpresa(req: Request, res: Response): Promise<Response> {
    if (req.body.IdMotorizadoEmpresa != undefined) {
        let idMotorizadoEmpresa = req.body.IdMotorizadoEmpresa;
        try {
            const conn = await database.conexionObtener();
            let resultado1 = await conn.query("DELETE FROM Empresa_Has_Motorizado WHERE IdEmpresaHasMotorizado = "+idMotorizadoEmpresa);    
...
export async function MotorizadoAsignarEmpresa(req: Request, res: Response): Promise<Response> {
    if(req.body.IdMotorizado != undefined && req.body.IdEmpresa != undefined){
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoMotorizado = await conn.query("SELECT IdEmpresaHasMotorizado FROM Empresa_Has_Motorizado WHERE IdEmpresa="+idEmpresa+" AND IdMotorizado="+idMotorizado);
            let resultadoMotorizadoFinal = JSON.parse(JSON.stringify(resultadoMotorizado[0]));

            if (resultadoMotorizadoFinal.length > 0) {
                return res.json({ success: false, msg:'Error, ya se encuentra asignada la empresa seleccionada'});
            } else {
                let insertar = await conn.query("INSERT INTO Empresa_Has_Motorizado (IdEmpresa,IdMotorizado,FechaReg,Anulado) "+
...
export async function EmpresaEditar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdEmpresa != undefined && req.body.Data != undefined){
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoAnuncio = await conn.query("SELECT ImagenUrl FROM Gen_Empresa WHERE IdEmpresa = "+idEmpresa);
            let resultadoAnuncioFinal = JSON.parse(JSON.stringify(resultadoAnuncio[0]));
            ...
            if(imagen == ''){
                let insertar = await conn.query("UPDATE Gen_Empresa SET IdEmpresaCategoria="+idCategoria+", IdEmpresaSubCategoria="+idSubCategoria+", RazonSocial = '"+razonSocial+"', Descripcion = '', Ruc = '"+ruc+"', Direccion = '"+direccion+"',"+
                ...
                if(archivoAnterior != '' && archivoAnterior != null){
                    fs.unlink(join(__dirname, '/../public/img/empresas/') + archivoAnterior, (err) => {
                    });
                }
                return res.json({ success: true });
            }
...
export async function EmpresaConfigurar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdEmpresa != undefined && req.body.Data != undefined){
       let idEmpresa = req.body.IdEmpresa
       ...
       let idRegion = '' + req.body.Data.IdRegion
        ...
        try {
            ...
            let insertar = await conn.query("UPDATE Gen_Empresa SET PrecioDelivery = "+precioBaseDelivery+",PrecioKm = "+precioPorKm+",KmArea = "+areaKm+",Latitud = '"+latitud+"',Longitud = '"+longitud+"',FacturaUrl = '"+facturaUrl+"',"+
...
export async function EmpresaNueva(req: Request, res: Response): Promise<Response> {
    if(req.body.Data != undefined){
        ...
        try {
            ...
            let insertar = await conn.query("INSERT INTO Gen_Empresa(IdEmpresaCategoria,IdEmpresaSubCategoria,RazonSocial,Ruc,Direccion,DistritoUbicacion,"+
...
export async function PedidoProceso(req: Request, res: Response): Promise<Response> {
...
export async function PedidoAsignar(req: Request, res: Response): Promise<Response> {
...
export async function PedidoCambiar(req: Request, res: Response): Promise<Response> {
...
export async function MotorizadosServicio(req: Request, res: Response): Promise<Response> {
    ...
    if (req.query.IdRegion != undefined) {
        ...
        try {
            let conn = await database.conexionObtener();
            let motorizadosRegion = await conn.query("SELECT IFNULL(Pedidos.PedidosAsignados,0) AS PedidosAsignados, Gen_Motorizado.IdMotorizado,Gen_Motorizado.Dni,Gen_Motorizado.Nombres, "+
...
export async function ProductosEmpresa(req: Request, res: Response): Promise<Response> {
    if(req.query.Busqueda != undefined && req.query.IdEmpresa != undefined  ){
        ...
        try {
            ...
            let rutaFinal = servidor + rutaAlmacenamiento;
            let conn = await database.conexionObtener();
            let resultadoProductos = await conn.query("SELECT Gen_Producto.IdProducto,Gen_Producto.IdProductoCategoria,Gen_Producto.Producto,Gen_Producto.ProductoDesc,Gen_Producto.PrecioContado,CONCAT('"+rutaFinal+"',Gen_Producto.Imagen) AS Imagen,Gen_Producto.UsuarioReg,Gen_Producto.Anulado,DATE_FORMAT(Gen_Producto.FechaReg, '%Y-%m-%d %H:%i:%s') AS FechaReg,Gen_ProductoCategoria.ProductoCategoria,Gen_ProductoCategoria.MostrarDelivery,Gen_ProductoCategoria.Anulado AS AnuladoCat "+
...
export async function ProductosCategoriasEmpresa(req: Request, res: Response): Promise<Response> {
    if(req.query.Busqueda != undefined && req.query.IdEmpresa != undefined  ){
        let idEmpresa = req.query.IdEmpresa;
        let busqueda = req.query.Busqueda || '';
        try {
            let conn = await database.conexionObtener();
            let resultadoCategoriasPro = await conn.query("SELECT IdProductoCategoria,IdEmpresa,ProductoCategoria,Anulado, "+
...
export async function ProductosCategoriaAgregar(req: Request, res: Response): Promise<Response> {
    if(req.body.ProductoCategoria != undefined && req.body.IdEmpresa != undefined){
        let idEmpresa = req.body.IdEmpresa;
        let productoCategoria = req.body.ProductoCategoria;
        try {
            let conn = await database.conexionObtener();
            let resultadoCategoriasPro = await conn.query("SELECT * FROM Gen_ProductoCategoria WHERE IdEmpresa = "+idEmpresa+"  AND ProductoCategoria ='"+productoCategoria+"'");
            let resultadoCategoriasProFinal = JSON.parse(JSON.stringify(resultadoCategoriasPro[0]));
            ...
            if (resultadoCategoriasProFinal.length > 0) {
                ...
                const hoy = moment().tz("America/Lima");
                const hoyformateadoMod = hoy.format('YYYY-MM-DD HH:mm:ss');
                let insertar = await conn.query("INSERT INTO Gen_ProductoCategoria (IdEmpresa,ProductoCategoria,Anulado,FechaReg,UsuarioReg,MostrarDelivery,Orden) "+
                " VALUES ("+idEmpresa+",'"+productoCategoria+"',0,'"+hoyformateadoMod+"','admin',0,0)");
                let resultadoCategoriasPro2 = await conn.query("SELECT * FROM Gen_ProductoCategoria WHERE IdEmpresa = "+idEmpresa);
...
export async function ProductoNuevo(req: Request, res: Response): Promise<Response> {
    if(req.body.IdEmpresa != undefined && req.body.Data != undefined){
        let idEmpresa = req.body.IdEmpresa
        ...
        try {
            ...
            let resultadoCategoriasPro = await conn.query("SELECT * FROM Gen_Producto "+
            ...
            if (resultadoCategoriasProFinal.length > 0) {
                ...
                if(imagen!=''){
                    ...
                    let insertar = await conn.query("INSERT INTO Gen_Producto (IdProductoCategoria,Producto,ProductoDesc,PrecioContado,Imagen,FechaReg,UsuarioReg,Anulado)"+
                    " VALUES ('"+idProductoCategoria+"','"+producto+"','"+productoDesc+"',"+precioContado+",'"+archivoNombre+"','"+hoyformateadoMod+"','ADMIN',0)");
                    return res.json({ success: true });
                }else{
                    let insertar = await conn.query("INSERT INTO Gen_Producto (IdProductoCategoria,Producto,ProductoDesc,PrecioContado,FechaReg,UsuarioReg,Anulado)"+
...
export async function ProductoEditar(req: Request, res: Response): Promise<Response> {
    if(req.body.IdEmpresa != undefined && req.body.Data != undefined){
        let idEmpresa = req.body.IdEmpresa
        ...
        try {
            ...
            let conn = await database.conexionObtener();
            let resultadoImagen = await conn.query("SELECT Imagen FROM Gen_Producto WHERE IdProducto = "+idProducto);
            ...
            if (resultadoCategoriasProFinal.length > 0) {
                ...
                const hoyformateadoMod = hoy.format('YYYY-MM-DD HH:mm:ss');
                if(imagen == ''){
                    let insertar = await conn.query("UPDATE Gen_Producto SET IdProductoCategoria ="+idProductoCategoria+" ,Producto = '"+producto+"' , ProductoDesc = '"+productoDesc+"', PrecioContado = "+precioContado+
                    ...
                    let insertar = await conn.query("UPDATE Gen_Producto SET IdProductoCategoria ="+idProductoCategoria+" ,Producto = '"+producto+"', ProductoDesc = '"+productoDesc+"', PrecioContado = "+precioContado+",Imagen = '"+archivoNombre+"'"+
                    " WHERE IdProducto ="+idProducto);
                    if(archivoAnterior != '' && archivoAnterior != null){
                        fs.unlink(join(__dirname, '/../public/img/productos/') + archivoAnterior, (err) => {
                        });
                    }
                    return res.json({ success: true });
                }
...
export async function ProductoAnular(req: Request, res: Response): Promise<Response> {
    if (req.body.IdProducto != undefined && req.body.Accion != undefined) {
        let idProducto = req.body.IdProducto;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado,Producto FROM Gen_Producto WHERE IdProducto = "+idProducto+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].Anulado;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_Producto SET  Anulado = "+accion+" WHERE IdProducto ="+idProducto);    
...
export async function ProductosCategoriaAnular(req: Request, res: Response): Promise<Response> {
    if (req.body.IdProductoCategoria != undefined && req.body.Accion != undefined) {
        let idProductoCategoria = req.body.IdProductoCategoria;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT Anulado,ProductoCategoria FROM Gen_ProductoCategoria WHERE IdProductoCategoria = "+idProductoCategoria+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].Anulado;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_ProductoCategoria SET  Anulado = "+accion+" WHERE IdProductoCategoria ="+idProductoCategoria);    
...
export async function ProductosCategoriaMostrar(req: Request, res: Response): Promise<Response> {
    if (req.body.IdProductoCategoria != undefined && req.body.Accion != undefined) {
        let idProductoCategoria = req.body.IdProductoCategoria;
        let accion = req.body.Accion;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT MostrarDelivery,ProductoCategoria FROM Gen_ProductoCategoria WHERE IdProductoCategoria = "+idProductoCategoria+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                let anulado = resultado[0].MostrarDelivery;
                if (anulado != accion) {
                    let resultado1 = await conn.query("UPDATE Gen_ProductoCategoria SET  MostrarDelivery = "+accion+" WHERE IdProductoCategoria ="+idProductoCategoria);    
...
export async function ProductosCategoriaOrden(req: Request, res: Response): Promise<Response> {
    if (req.body.IdProductoCategoria != undefined && req.body.Orden != undefined) {
        let idProductoCategoria = req.body.IdProductoCategoria;
        let orden = req.body.Orden;
        try {
            const conn = await database.conexionObtener();
            const empresa = await conn.query("SELECT IdProductoCategoria FROM Gen_ProductoCategoria WHERE IdProductoCategoria = "+idProductoCategoria+" LIMIT 1");
            let resultado = JSON.parse(JSON.stringify(empresa[0]));
            if (resultado.length > 0) {
                    let resultado1 = await conn.query("UPDATE Gen_ProductoCategoria SET  Orden = "+orden+" WHERE IdProductoCategoria ="+idProductoCategoria);    
...
export async function NotificacionesFcm(req: Request, res: Response): Promise<Response> {
    if(req.query.Tipo != undefined){
        ...
        try {
            let conn = await database.conexionObtener();
            let resultadoNotificacion = await conn.query("SELECT Gen_NotificacionFcm.IdNotificacionFcm,Gen_NotificacionFcm.TipoFcm,Gen_NotificacionFcm.Topico,Gen_NotificacionFcm.IdEmpresa,Gen_NotificacionFcm.Titulo,Gen_NotificacionFcm.Msg,"+
...
export async function EnviarNuevaNotificacion(req: Request, res: Response): Promise<Response> {
            ...
            try {
                switch (opcion) {
                    case 1:{
                        ...
                        const hoyformateado = hoy.format('YYYY-MM-DD HH:mm:ss');
                        let conn = await database.conexionObtener();
                        let insertar = await conn.query("INSERT INTO Gen_NotificacionFcm(TipoFcm,Titulo,Msg,FechaEmision,IdUsuarioReg)"+
...
/* INICIO DE PEDIDOS */
export async function PedidosTotal(req: Request, res: Response): Promise<Response> {
...
export async function ReportePedidos(req: Request, res: Response): Promise<Response> {
